...
def data_to_clss_parts(data, use_images=False, setting='simple'):
    if use_images and setting == 'simple':
        return data[1]
    elif use_images and setting == 'double':
        return data[2]
    return data[2]
...
def test_and_save(dset, seed, prefix, model, opt, test_dl, cuda, n_obj, test_idx=0,
                  use_images=False, setting='simple'):
    l, a = one_step(
        model,
        opt,
        test_dl,
...
def one_step(model,
    ...
    for data in tqdm(dl):
        ...
        clss = clss_fn(data, use_images=use_images, setting=setting)
        if cuda:
            clss = clss.cuda()
        if train:
...
def model_metrics(expe_idx, dir_prefix='', n_test=0, var='std'):
    ...
    for m_str in config['models']:
        ...
        mdata = []
        for p in d_paths:
            s = re.search(r'^{}_([0-9]+)_([0-9]+)_test_acc.npy$'.format(n_test), p)
            if s:
                # file name, dataset number, seed number
                mdata.append((s[0], int(s[1]), int(s[2])))
...
def test_on(expe_idx, tpath, midx=0, dir_prefix='', var='std', use_images=False, setting='simple'):
    """
