...
class Gen():
    ...
    def to_dataset(self, n=None, label_type='long', device=torch.device('cpu'), use_images=False):
        """
...
class SameConfigGen(Gen):
    ...
    def render(self, path):
        # we know that for simple the number of objs is fixed
        num_obj = sum(torch.tensor(self.t_batch) == 0).item()
        vecs = np.array(self.targets)
        for i in tqdm(range(self.t_batch[-1] + 1)): # number of samples
            start_idx = i * num_obj
            end_idx = (i + 1) * num_obj
            state_list = list(vecs[start_idx:end_idx])
            self.env.from_state_list(state_list, norm=True)
            img = self.env.render(mode='fixed', show=False)
            cv2.imwrite(op.join(path, 'img_%s.jpg' % i), img)
            self.env.reset()
...
class CompareConfigGen(Gen):
    ...
    def render(self, path):
        ...
        num_samples = self.t_batch[-1] + 1
        tvecs = np.array(self.targets)
        rvecs = np.array(self.refs)
        max_idx = len(self.t_batch)
        start_idx = 0
        end_idx = 0
        for sample_idx in tqdm(range(num_samples)):
            while self.t_batch[end_idx] == sample_idx and end_idx < max_idx:
                end_idx += 1
                if end_idx == max_idx:
                    break
            t_state_list = tvecs[start_idx:end_idx]
            r_state_list = rvecs[start_idx:end_idx]
            self.env.from_state_list(t_state_list, norm=True)
            t_img = self.env.render(show=False, mode='fixed')
            cv2.imwrite(op.join(path, '%s_target.jpg' % sample_idx), t_img)
            self.env.reset()
            self.env.from_state_list(r_state_list, norm=True)
            r_img = self.env.render(show=False, mode='fixed')
            cv2.imwrite(op.join(path, '%s_reference.jpg' % sample_idx), r_img)
            self.env.reset()
        ...
        # num_obj = sum(torch.tensor(self.t_batch) == 0).item()
        # vecs = np.array(self.targets)
        # for i in range(self.t_batch[-1] + 1): # number of samples
        #     start_idx = i * num_obj
        #     end_idx = (i + 1) * num_obj
        #     state_list = list(vecs[start_idx:end_idx])
        #     self.env.from_state_list(state_list, norm=True)
